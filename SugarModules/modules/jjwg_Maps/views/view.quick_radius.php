...
<table border="0">
    ...
    <tr>
        <td>
            <strong><?php echo $mod_strings['LBL_MODULE_TYPE'].' '; ?> </strong>
        </td>
        <td>
            <select id="display_module" tabindex="111" title="<?php echo $mod_strings['LBL_MODULE_TYPE']; ?>" name="display_module">
                <?php foreach ($valid_geocode_modules as $module) { ?>
    ...
    <tr>
        <td>
            <strong><?php echo $mod_strings['LBL_DISTANCE'].' '; ?> </strong>
        </td>
        <td>
            <input type="text" name="distance" id="distance" 
            value="<?php echo htmlspecialchars($_REQUEST['distance']); ?>" title='' tabindex='121' size="10" maxlength="10">
            <select id="unit_type" tabindex="131" title="<?php echo $mod_strings['LBL_DISTANCE']; ?>" name="unit_type">
                <?php foreach ($app_list_strings['map_unit_type_list'] as $key=>$value) { ?>
...
<br />
<input class="button" tabindex="211" type="submit" name="submit" value="<?php echo $mod_strings['LBL_MAP_PROCESS']; ?>" align="bottom">
</form>
